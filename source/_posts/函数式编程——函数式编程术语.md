...
tags: [函数式编程, 范畴论, JavaScript]
categories: 函数式编程
description: 介绍函数式编程中的一些术语（基于 JavaScript)
---
...
- [Arity](#arity)
- [高阶函数 (Higher-Order Function / HOF)](#高阶函数-higher-order-function--hof)
- [闭包 (Closure)](#闭包-closure)
- [偏函数应用 (Partial Application)](#偏函数应用-partial-application)
- [柯里化 (Currying)](#柯里化-currying)
- [自动柯里化 (Auto Currying)](#自动柯里化-auto-currying)
    - [进一步阅读](#进一步阅读)
- [函数组合 (Function Composition)](#函数组合-function-composition)
- [Continuation (后续)](#continuation-后续)
- [纯函数 (Purity)](#纯函数-purity)
- [副作用 (Side effects)](#副作用-side-effects)
- [幂等 (Idempotent)](#幂等-idempotent)
- [Point-Free 风格 (Point-Free Style)](#point-free-风格-point-free-style)
- [断定 (Predicate)](#断定-predicate)
- [契约 (Contracts)](#契约-contracts)
- [范畴 (Category)](#范畴-category)
    - [进一步阅读](#进一步阅读-1)
- [值 (Value)](#值-value)
- [常量 (Constant)](#常量-constant)
- [函子 (Functor)](#函子-functor)
  - [一致性 (Preserves identity)](#一致性-preserves-identity)
  - [组合性 (Composable)](#组合性-composable)
  - [指向函子 (Pointed Functor)](#指向函子-pointed-functor)
- [抬升 (Lift)](#抬升-lift)
- [引用透明性 (Referential Transparency)](#引用透明性-referential-transparency)
- [等式推理 (Equational Reasoning)](#等式推理-equational-reasoning)
- [Lambda](#lambda)
- [Lambda演算 (Lambda Calculus)](#lambda演算-lambda-calculus)
- [惰性求值 (Lazy evaluation)](#惰性求值-lazy-evaluation)
- [幺半群 (Monoid)](#幺半群-monoid)
- [单子 (Monad)](#单子-monad)
- [余单子 (Comonad)](#余单子-comonad)
- [应用函子 (Applicative Functor)](#应用函子-applicative-functor)
- [态射 (Morphism)](#态射-morphism)
  - [Endomorphism (自同态)](#endomorphism-自同态)
  - [Isomorphism (同构)](#isomorphism-同构)
  - [Homomorphism (同态)](#homomorphism-同态)
...
- [Setoid](#setoid)
- [半群 (Semigroup)](#半群-semigroup)
- [可折叠性 (Foldable)](#可折叠性-foldable)
- [透镜 (Lens)](#透镜-lens)
- [类型签名 (Type Signatures)](#类型签名-type-signatures)
- [代数数据类型 (Algebraic data type)](#代数数据类型-algebraic-data-type)
  - [和类型 (Sum type)](#和类型-sum-type)
  - [Product type](#product-type)
- [可选类型 (Option)](#可选类型-option)
- [Function](#function)
- [偏函数 (Partial function)](#偏函数-partial-function)
  - [处理偏函数](#处理偏函数)
- [在 js 中的函数式编程库](#在-js-中的函数式编程库)
